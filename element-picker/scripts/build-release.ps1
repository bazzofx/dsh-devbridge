...
excludes development-only folders (docs/, demo/, scripts/, releases/, website/).
...
$jsFiles = Get-ChildItem -Path (Join-Path $ProjectDir 'content'), (Join-Path $ProjectDir 'popup'), (Join-Path $ProjectDir 'options'), (Join-Path $ProjectDir '*.js') -Filter '*.js' -ErrorAction SilentlyContinue
...
try {
    ...
    try {
        ...
        $flat = ($names -join '|').Replace('\', '/')
        if ($flat -notmatch 'options/options\.html') { throw 'Zip is missing options/options.html' }
        Write-Host "Zip entries: $($names.Count) (manifest.json + options at root: yes)"
    } finally { $zip.Dispose() }
