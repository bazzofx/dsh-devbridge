...
(() => {
  ...
  $('#authorLink').addEventListener('click', (e) => {
    e.preventDefault();
    chrome.tabs.create({ url: 'https://dev-bridge.cybersamurai.co.uk' });
    window.close();
  });

  // Buy me a coffee — open in a new tab (extension pages cannot navigate themselves).
  $('#bmcBtn').addEventListener('click', () => {
    chrome.tabs.create({ url: 'https://www.buymeacoffee.com/bazzofx' });
    window.close();
  });
